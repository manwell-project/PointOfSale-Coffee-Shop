...
async function init() {
    ...
    // Check if redirected from scanner page

    const urlParams = new URLSearchParams(window.location.search);

    const scannedSku = urlParams.get('sku');
    ...
    if (scannedSku) {
        ...
        try {
            ...
            const stock = await window.API.Stocks.getBySku(scannedSku);

            openModal(stock.id);

            showNotification('Barang ditemukan di database', 'success');
            ...
            if (err.message && err.message.includes('404')) {
                ...
                showNotification('Barang baru, silakan lengkapi data', 'info');
